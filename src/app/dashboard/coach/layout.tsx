...
export default function CoachLayout({
  ...
  return (
    <div className="flex min-h-screen bg-background overflow-hidden">
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-nav-safe border-t border-border/30 bg-background/90 backdrop-blur-xl px-4 flex items-center justify-around z-50 pb-safe">
        ...
        <MobileNavItem href="/dashboard/coach/library" icon={Library} label="Librería" />
      </nav>
    </div>
  )
}
...
function MobileNavItem({ href, icon: Icon, label }: { href: string, icon: any, label: string }) {
  return (
    <Link href={href} className="flex flex-col items-center justify-center gap-1 min-w-[50px] py-2 text-muted-foreground hover:text-primary transition-colors">
